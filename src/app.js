...
function buildPath(spec, params) {
  return spec.replace(/{([^}]+)}/g, (_, k) => {
    return params[k] !== undefined
      ...
      : macros[k] !== undefined
      ? macros[k]()
      : DateTime.utc().toFormat(k)
  })
}

function formatBody(body, { final_newline: finalNewLine, format }) {
  if (format === 'json') {
    ...
    return (
      body.filter(item => typeof item === 'string').join('\n') +
      (finalNewLine ? '\n' : '')
    )
  } else {
    return ''
...
module.exports = async log => {
  ...
  const handleRequest = async ({ body, id, params }, spec, p) => {
  ...
  app.eval = async p => {
    ...
    fastify.addHook('preHandler', (request, reply, done) => {
      if (request.headers.authorization === p.secret) done()
      else {
        reply.code(401)
    ...
    fastify.post('/:name(^\\w+)', { schema }, async request => {
